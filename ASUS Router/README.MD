...
Step 1. Getting the list of YouTube IPs. 

It's hard. I got the list from here https://rockblack.su/vpn/dopolnitelno/diapazon-ip-adresov

Step 2. Convert the list to VPN Director's format. Replace 192.168.7.0/24 with your local IP!
```bash
cat << 'EOF' > /tmp/rules
142.250.184.0/24
107.0.0.0/8
...
Step 3. Overwrite the existing VPN Director configuration file. Existing records will be deleted! You can use ">>" instead of ">" to append
```bash
...
ZONE_ID="aaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" # get it from your Cloudflare dashboard
RECORD_ID="bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb" # curl ... "https://api.cloudflare.com/client/v4/zones/${ZONE_ID}/dns_records" | json_pp
...
# Update NEXT DNS
curl -fs -o /dev/null "https://link-ip.nextdns.io/123123/123123"
```
